<?php
/**
 * Plugin Name: Fudugo Woo Gift Coupon
...
function woocommerce_gift_coupon_send_action($post_ids){
    ...
    foreach ( $post_ids as $post_id ) {
        ...
        foreach ( $items as $key => $item ) {
		    ...
		    $mailv = wc_get_order_item_meta($item->get_id(), 'recipient email');

		    //echo '<pre>'; print_r($mailv); echo '</pre>';
		    ...
		    if($mailv == ''){
                    ...
                    if($_product->product_type == 'variation'){
                        //$cbn = $item['pa_select-gift-value'];
                        $cbn = wc_get_order_item_meta($item->get_id(), 'pa_select-gift-value'); 
                    }else {
                        $cbn = $amount[0];
                    }
                    ...
                    update_post_meta( $new_coupon_id, 'coupon_amount', $cbn );
    ...
    switch ( $column ) {
        ...
        case 'coupon_status' :
            ...
            if ( $coupons_generated > 0 ) {
                printf( '<span class="%s">%s</span>', sanitize_title( "Sent" ), __( "Sent", 'woocommerce-gift-coupon' ) );
            }else{
...
function render_meta_on_cart_item( $title = null, $cart_item = null, $cart_item_key = null ) {
    if( $cart_item_key && is_cart() ) {
        ...
        if (strpos($title , "E-GIFT CARD") > -1) {
            $mailto = empty($cart_item[ 'recipient_email']) ? "Self" : $cart_item[ 'recipient_email'];
            echo $title. '<dl class="">
                 <dt class="">Mail To : </dt>
                 <dd class=""><p>'.$mailto .'</p></dd>           
              </dl>';
              //echo '<pre>'; print_r($cart_item ); echo '</pre>';
//           echo "key:".$cart_item_key;
        }else{
            echo $title;
        }
...
function tshirt_order_meta_handler( $item_id, $values, $cart_item_key ) {
    wc_add_order_item_meta( $item_id, "recipient_email", WC()->session->get( $cart_item_key.'recipient_email') );    
}
...
function disable_plugin_updates( $value ) {
  if ( isset($value) && is_object($value) ) {
    if ( isset( $value->response['woocommerce-gift-coupon/woocommerce-gift-coupon.php'] ) ) {
      unset( $value->response['woocommerce-gift-coupon/woocommerce-gift-coupon.php'] );
    }
  }
  return $value;
}
